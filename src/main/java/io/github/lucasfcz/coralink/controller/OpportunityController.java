...
public class OpportunityController {
    ...
    public ResponseEntity<Page<OpportunityResponse>> getOpportunities(
            ...
            @RequestParam(required = false) Set<TargetAudience> targetAudience,
            @RequestParam(required = false) Modality modality,
            @RequestParam(required = false) Boolean isFree,
            Pageable pageable) {
        return ResponseEntity.ok(opportunityService.getRelevantOpportunities(type, thematicArea, targetAudience, modality, isFree, pageable));
